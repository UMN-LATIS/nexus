...
public:
    ...
    void createDecodingTables1();
    void createEncodingTables();
    void createEncodingTables1();
    ...
    unsigned char *compress(unsigned char *data, int input_size, int &output_size);
        unsigned char *compress1(unsigned char *data, int input_size, int &output_size);
    ...
    void append(int old_index, int new_index, int length, uchar symbol);
    void dump(int count);
    void check(int node, std::vector<unsigned char> &word);
    ...
    int wordsize;
    int dictionarysize;
    std::vector<uint32_t> probs; //proabilitiy table
    ...
    struct Node {
        Node(int c = 0, int i = 0): first_child(c), index(i) {}
        int first_child; //-1 means no children, -2 means removed
        int index;
    };
